...
; transform. The replacement handle and getpointer are ordinary (non-PHI)
; instructions and must be anchored at the first non-PHI instruction of the
; block. Otherwise they would be inserted before %ptr_phi - and therefore ahead
; of the surviving %sibling PHI - producing an illegal module where PHI nodes
; are no longer grouped at the top of the block. Note: verification is
; intentionally left enabled so a regression is caught by the verifier.
...
; CHECK:     main:
; The PHIs must stay grouped at the top of the block, with the non-PHI handle
; and access emitted only after the last PHI.
...
main:
  ; %ptr_phi is resolved to indices; %sibling is an unrelated PHI that comes
  ; after it in the block and must remain grouped with the other PHIs.
