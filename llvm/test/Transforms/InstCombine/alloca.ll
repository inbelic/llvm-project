...
; RUN: opt < %s -passes=instcombine -S -data-layout="E-p:64:64:64-a0:0:8-f32:32:32-f64:64:64-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:32:64-n64-v64:64:64-v128:128:128" | FileCheck %s -check-prefixes=ALL,CHECK
; RUN: opt < %s -passes=instcombine -S -data-layout="E-p:32:32:32-a0:0:8-f32:32:32-f64:64:64-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:32:64-n64-v64:64:64-v128:128:128" | FileCheck %s -check-prefixes=ALL,P32
; RUN: opt < %s -passes=instcombine -S -data-layout="n8:16:32:64" | FileCheck %s -check-prefixes=ALL,NOPTR
...
;
; NOPTR-LABEL: @test(
; NOPTR-NEXT:    [[X:%.*]] = alloca [0 x i32], align 8
; NOPTR-NEXT:    call void (...) @use(ptr nonnull [[X]])
; NOPTR-NEXT:    call void (...) @use(ptr nonnull [[X]])
; NOPTR-NEXT:    call void (...) @use(ptr nonnull [[X]])
; NOPTR-NEXT:    call void (...) @use(ptr nonnull [[X]])
; NOPTR-NEXT:    ret void
...
;
; NOPTR-LABEL: @test4(
; NOPTR-NEXT:    [[TMP1:%.*]] = zext i32 [[N:%.*]] to i64
; NOPTR-NEXT:    [[A:%.*]] = alloca i32, i64 [[TMP1]], align 4
; NOPTR-NEXT:    ret ptr [[A]]
;
  %A = alloca i32, i32 %n
  ret ptr %A
}
...
; NOPTR-LABEL: @test6(
; NOPTR-NEXT:  entry:
; NOPTR-NEXT:    [[A:%.*]] = alloca { i32 }, align 8
; NOPTR-NEXT:    [[B:%.*]] = alloca i32, align 4
; NOPTR-NEXT:    store volatile i32 123, ptr [[A]], align 4
; NOPTR-NEXT:    tail call void @f(ptr nonnull [[B]])
; NOPTR-NEXT:    ret void
;
...
; NOPTR-LABEL: @test9(
; NOPTR-NEXT:  entry:
; NOPTR-NEXT:    [[ARGMEM:%.*]] = alloca inalloca <{ [[STRUCT_TYPE:%.*]] }>, align 8
; NOPTR-NEXT:    [[TMP0:%.*]] = load i64, ptr [[A:%.*]], align 4
; NOPTR-NEXT:    store i64 [[TMP0]], ptr [[ARGMEM]], align 8
; NOPTR-NEXT:    call void @test9_aux(ptr nonnull inalloca(<{ [[STRUCT_TYPE]] }>) [[ARGMEM]])
; NOPTR-NEXT:    ret void
;
entry:
