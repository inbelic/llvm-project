...
Value mlir::spirv::linearizeIndex(ValueRange indices, ArrayRef<int64_t> strides,
  ...
  for (const auto &index : llvm::enumerate(indices)) {
    ...
    Value update =
        builder.createOrFold<spirv::IMulOp>(loc, strideVal, index.value());
    linearizedIndex =
        builder.createOrFold<spirv::IAddOp>(loc, linearizedIndex, update);
    // Following works as expected by switching operand order:
    // Value update =
